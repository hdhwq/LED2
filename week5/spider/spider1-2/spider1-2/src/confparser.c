...
void module_push_back(Config *conf, char *str)
{
	if(NULL == str)
	{
	...
	//conf->mcnt++;
	conf->modules = (char **)realloc(conf->modules, (conf->mcnt+1) * sizeof(char *));
	conf->modules[conf->mcnt] = strdup(str);
	conf->mcnt++;
...
void push_back(Config *conf, char *str)
{
	if(NULL == str)
	{
		SPIDER_LOG(SPIDER_LEVEL_ERROR, "param is null\n");
	...
	{
		conf->modules = (char **)malloc(sizeof(char *) * ITEMS);
		if(NULL == conf->modules)
		{
			SPIDER_LOG(SPIDER_LEVEL_ERROR, "Can't malloc modules\n");
			return ;
		}
		memset(conf->modules, 0, sizeof(char *) * ITEMS);
		conf->mcnt = 0;
	}
	
	//if(mcnt==ITEMS)
	int len = strlen(str)+1;
	conf->modules[conf->mcnt] = (char *)malloc(len);
	if(NULL == conf->modules[conf->mcnt])
	{
			SPIDER_LOG(SPIDER_LEVEL_ERROR, "Can't malloc\n");
			return ;  // exit...
	}
	stpcpy(conf->modules[conf->mcnt], str);
	conf->mcnt++;
		
}

void display_modules(Config *conf)
{
	int i;
	for(i=0; i<conf->mcnt; i++)
		SPIDER_LOG(SPIDER_LEVEL_DEBUG, "module:%s\n", conf->modules[i]);
}
    ...
    while (fgets(buf, MAX_CONF_LEN+1, fp) != NULL) {
        ...
        if (argc == 2) {
            if (strcasecmp(argv[0], "max_job_num") == 0) {
				...
				//push_back(g_conf, strdup(argv[1]));
				module_push_back(g_conf, argv[1]);
